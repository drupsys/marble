...
class Layer extends Unit {
    ...
    @test "should throw an error if input shape doesn't match layer shape" () {
        ...
        expect(() => { this.layer.forward(input, false)}).to.throw(layer.LayerException)
    }

    @test "should not throw an error if input shape matches layer shape" () {
        let input = linear.Matrix.random(1, 2)
        expect(() => { this.layer.forward(input, false)}).to.not.throw(layer.LayerException)
    }

    @test "should produce the output of the right shape" () {
        let input = linear.Matrix.random(1, 2)
        expect(this.layer.forward(input, false).shape.toString()).to.be.eq([1, 3].toString())
    }

    @test "should produce the right result" () {
        let tmp = layer.create(new layer.Layer(2, 1), [[0.5]], [[0.125], [0.35]])
        let input = new linear.Matrix([[5, 2]])
        expect(tmp.forward(input, false).get(0, 0)).to.be.eq(1.825)
    }

    @test "hypothesis function should to affect the output" () {
        let h: layer.Operator = (x) => { return 2 * x }
        let tmp = layer.create(new layer.Layer(2, 1, h), [[0.5]], [[0.125], [0.35]])
        
        let input = new linear.Matrix([[5, 2]])
        expect(tmp.forward(input, false).get(0, 0)).to.be.eq(2 * 1.825)
    }
